...
public class DelCarModel {
    public void delCar(int uid, int pid, String token, final OnNetListener onNetListener){
        ApiService apiService = RetrofitUtil.getInstance().getApiService(Api.devIP, ApiService.class);
        Call<BaseBean> call = apiService.delCart(uid, pid, token);
